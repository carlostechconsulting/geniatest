...
{
	public IQueryable ApplyQuery<T>(IQueryable<T> items, IQueryCollection query = null)
		where T : class
	{
		if (query != null)
		{
			...
			{
				items = (IQueryable<T>)DynamicQueryableExtensions.Skip(items, int.Parse(query["$skip"].ToString()));
			}

			if (query.ContainsKey("$top"))
			{
				items = (IQueryable<T>)DynamicQueryableExtensions.Take(items, int.Parse(query["$top"].ToString()));
			}
